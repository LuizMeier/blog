...
This is the second post I am writing about Backstage. Check out [my previous post about integrating with Entra ID](https://blog.lmeier.net/posts/authentication-backstage-entra-id/). Now, let's walk through all the steps to integrate Backstage and Azure DevOps to automate resource delivery in Azure using a CI/CD pipeline. Be prepared, because this one is going to be long!
  ...
  steps:
    ...
    - id: pushAzureRepo
      ...
      input:
        ...
        gitCommitMessage: Add name: ${{ parameters.name }} project files
...
In other words, it always concatenates the path you provide with the path from where the template was imported. Thus, either you keep the files to be handled in the same location as the file from which you imported the template or use a URL from an external location, which is the approach I used here in the `fetch:template` action. More about this can be seen in the [software templates documentation](https://backstage.io/docs/features/software-templates/) and the [CLI templates documentation](https://backstage.io/docs/tooling/cli/templates/).
</Callout>
